...
filename='python编程从入门到实践项目/data_visualization/data/death_valley_2018_simple.csv'
with open(filename) as f:
    ...
    for row in reader:
        ...
        try:
            ...
            print(f"Missing data for {current_date}") #这样的写法可以帮我们找点最高气温最低气温哪里有缺失值
        else:
            dates.append(current_date)
            highs.append(high)
            lows.append(low)
    #有点奇怪啊，这些操作都要再缩进情况下才能完成吗
    #如果try正常进行，就会执行else (那为什么不直接把else放在try的后面呢)
...
#设置图形的格式
title='the highest and lowest temperature on every day of 2018\n the death valley'
...
plt.savefig('death_valley temp.png')
